...
def test_streaming(args, model, test_loader, device, chunk_size=1):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc=f"Streaming (chunk={chunk_size})"):
            ...
            min_len = min(Z1_time.shape[1], R_gt_speech.shape[0])
            speech_es = Z1_time[0, :min_len]
            music_es = Z2_time[0, :min_len]
            others_es = Z3_time[0, :min_len]
            speech_gt = R_gt_speech[:min_len]
            music_gt = R_gt_music[:min_len]
            others_gt = R_gt_others[:min_len]
            
            speech_sisdr = sisdr_cost(speech_es.unsqueeze(0), speech_gt.unsqueeze(0))
            music_sisdr = sisdr_cost(music_es.unsqueeze(0), music_gt.unsqueeze(0))
            others_sisdr = sisdr_cost(others_es.unsqueeze(0), others_gt.unsqueeze(0))
            total_sisdr = (speech_sisdr + music_sisdr + others_sisdr) / 3
            
            speech_sisdr_list.append(speech_sisdr.item())
            music_sisdr_list.append(music_sisdr.item())
            others_sisdr_list.append(others_sisdr.item())
            total_sisdr_list.append(total_sisdr.item())
            
            if idx.item() < args.save_n_samples:
                save_dir = os.path.join(args.output_dir, f"sample{idx.item()}")
                os.makedirs(save_dir, exist_ok=True)
                wav_write(mixture.squeeze(), save_dir, "mixture.wav", fs)
                wav_write(speech_gt.squeeze(), save_dir, "speech_gt.wav", fs)
                wav_write(music_gt.squeeze(), save_dir, "music_gt.wav", fs)
                wav_write(others_gt.squeeze(), save_dir, "others_gt.wav", fs)
                wav_write(speech_es.squeeze(), save_dir, "speech_es.wav", fs)
                wav_write(music_es.squeeze(), save_dir, "music_es.wav", fs)
                wav_write(others_es.squeeze(), save_dir, "others_es.wav", fs)

    avg_speech_sisdr = np.mean(speech_sisdr_list)
    avg_music_sisdr = np.mean(music_sisdr_list)
    avg_others_sisdr = np.mean(others_sisdr_list)
    avg_total_sisdr = np.mean(total_sisdr_list)
    avg_time = np.mean(inference_times)

    print(f"Speech SI-SDR: {avg_speech_sisdr:.4f}")
    print(f"Music SI-SDR:  {avg_music_sisdr:.4f}")
    print(f"Others SI-SDR: {avg_others_sisdr:.4f}")
    print(f"Total SI-SDR:   {avg_total_sisdr:.4f}")
    print(f"Average inference time per sample: {avg_time*1000:.2f} ms")
    
    return avg_total_sisdr, avg_time
...
def test(args):
    ...
    streaming_sisdr, avg_time = test_streaming(args, model, test_loader, device, chunk_size=args.chunk_size)
