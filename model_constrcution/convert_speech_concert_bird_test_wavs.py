...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Copy generate_test_wavs_speech_concert_bird.py output into a "
            "dataset_root/test/{mixture,speech,music,noise}/... layout."
        )
    )
    parser.add_argument(
        "--input_dir",
        type=Path,
        required=True,
        help="Directory containing sampleN folders generated by generate_test_wavs_speech_concert_bird.py",
    )
    parser.add_argument(
        "--dataset_root",
        type=Path,
        required=True,
        help="Destination dataset root. Files are copied under dataset_root/test",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def destination_path(dataset_root, category, index):
    return (
        dataset_root
        / "test"
        / category
...
def main():
    ...
    for index, sample_dir in sample_dirs:
        ...
        for src_name, dst_category in FILE_MAPPINGS:
            ...
            dst_path = destination_path(args.dataset_root, dst_category, index)
            result = copy_one(
                src_path,
                dst_path,
